...
} 2> >(tee -a $outputDir/logs/pre_fastqc_error.log) > >(tee -a $outputDir/logs/pre_fastqc.log) #tee comand allows us to redirect stdout/stderr to the terminal and to a file at the same time.